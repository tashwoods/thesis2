...
The RS Bulk model is motivated by resolving the hierarchy problem, but this theory alone does not address all of the outstanding issues of the SM. There are many other interesting and well motivated new physics frameworks that address the other issues of the SM, but there is a lack of completely predictive models, due to model flexibility (free parameters). Therefore it is difficult for experimentalists to know which theories to search for in data. However, as seen in \cite{hvt}, a simplified model approach may be used when searching for narrow width resonances . For the narrow width resonances this search probes, the details and free parameters of a given theory . Generally such searches are only sensitive to the resonance mass and its interactions. Therefore, a theory's Lagrangian may be reduced to only retain this information (mass parameters and couplings). Experimental results using this framework may then be reinterpreted in a given theory. 

In the simplified approach, the new resonance searched for is represented as an additional heavy vector triplet (HVT), which is a real vector field in the adjoint representation of $SU(2)_{L}$ with vanishing hypercharge. This results in one neutral and two charged bosons, defined as:
